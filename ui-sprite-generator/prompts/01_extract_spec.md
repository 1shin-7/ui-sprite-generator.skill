...
- Use `schema_version: "1.2"`.
- `components[]` describes reusable sprite definitions. `instances[]` describes every placement of those sprites in the source layout.
- Identify every distinct reusable UI chrome component definition. Do not include plain text unless the text is a graphical sprite.
- For every visible placement, create an `instances[]` entry with a precise `source_bbox` in source image pixels and a `uses` reference to `components[].id`.
- `instances[].source_bbox` values may overlap. Source_bbox values may overlap by design. Do not shift, shrink, or offset a bbox just to avoid overlap with another component, text, badge, floating overlay, or neighboring UI.
...
- Merge only exact reusable sprite definitions: same role, same material, same ornament, same glow, same border, same center behavior, same state, same render pattern, same render params, and same attached decoration requirements.
- Do not merge near-duplicates, mirrored or rotated variants, state variants, lighting or shadow direction variants, components with unique badges or attached decorations, or components whose equality depends on hidden or occluded guesswork.
- When uncertain whether two UI elements are exactly identical, keep separate component definitions.
- Repeated exact components must share one component definition and use multiple lightweight `instances[]` entries.

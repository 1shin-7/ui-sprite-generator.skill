...
4. **Generate labeled atlas sheets** with `prompts/03_generate_ui_atlas.md`.
   ...
   For dense or multi-atlas work, run `scripts/plan_atlas_layout.py --from-spec ui-sprite-runs/YYYY-MM-DD-slug/spec.yaml --atlas 1536x1024 --component-group buttons --padding 24 --scale 2` before building the prompt. It uses MaxRects to emit stdout JSON placement guidance so the atlas layout is deterministic, but this output is not a stable contract and must not be promoted into a saved plan artifact.
